...
class ShowVisitActivitiesDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      title: Text("Activities Completed", style: Theme.of(context).textTheme.titleMedium,),
      content: SingleChildScrollView(
        child: visitActivities.isEmpty ? Text("No activities completed") : Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisSize: MainAxisSize.min,
          children: visitActivities.map((activity) => Padding(
            padding: const EdgeInsets.symmetric(vertical: 4.0),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(activity.description, style: Theme.of(context).textTheme.titleSmall),
                Text(formatDateTime(activity.createdAt), style: Theme.of(context).textTheme.labelSmall)
              ],
            ),
          )).toList(),
        ),
